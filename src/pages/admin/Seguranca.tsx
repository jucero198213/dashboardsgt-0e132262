...
export default function Seguranca() {
  ...
  const Toggle = ({ label, desc, checked, onToggle }: { label: string; desc: string; checked: boolean; onToggle: () => void }) => (
    <div className="flex items-center justify-between gap-4 py-2.5 border-b border-white/5 last:border-0">
      ...
      <button onClick={onToggle} className={`relative h-5 w-9 rounded-full transition-colors duration-200 shrink-0 ${checked ? "bg-cyan-500" : "bg-white/10"}`}>
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <div className="rounded-[20px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,26,53,0.72)_0%,rgba(11,17,35,0.94)_100%)] p-5 space-y-4">
          <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-400 flex items-center gap-2">
          ...
          <div className="rounded-[12px] border border-cyan-500/20 bg-cyan-500/5 px-4 py-3 space-y-1.5">
            ...
            <p className="text-xs text-slate-500">ID: <span className="font-mono">{user?.id?.substring(0, 16)}…</span></p>
        ...
        <div className="rounded-[20px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,26,53,0.72)_0%,rgba(11,17,35,0.94)_100%)] p-5 space-y-3">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-slate-300">Comprimento mínimo</span>
              <span className="text-sm font-bold text-white">{pwdLen} caracteres</span>
            </div>
        ...
        <div className="rounded-[20px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,26,53,0.72)_0%,rgba(11,17,35,0.94)_100%)] p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={addIP}
              className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-2.5 py-1 text-xs text-slate-400 hover:text-white transition-all">
              <Plus className="h-3 w-3" /> Adicionar
            </button>
          </div>
          <div className="space-y-2">
            {ips.map((ip, i) => (
              <div key={i} className="flex items-center justify-between rounded-[10px] border border-white/6 bg-white/[0.025] px-3 py-2">
