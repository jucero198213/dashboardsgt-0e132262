...
export default function Configuracoes() {
  ...
  const save = () => {
    // Persiste tunnel URL no sessionStorage para uso na dwApi
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <div className="rounded-[20px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,26,53,0.72)_0%,rgba(11,17,35,0.94)_100%)] p-5 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-400">Tunnel URL — DW Local</p>
          </div>
          <p className="text-xs text-slate-500">URL do Cloudflare Tunnel que conecta o portal ao servidor Node.js local. Atualizar quando reiniciar o tunnel.</p>
          <input
            value={tunnelUrl}
            onChange={(e) => setTunnelUrl(e.target.value)}
            className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white font-mono placeholder:text-slate-600 focus:outline-none focus:border-cyan-500/50"
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => window.open(tunnelUrl + "/health", "_blank")}
              className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm text-slate-400 hover:text-white transition-all">
        ...
        <div className="rounded-[20px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,26,53,0.72)_0%,rgba(11,17,35,0.94)_100%)] p-5 space-y-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-400">Feature Flags</p>
          </div>
          {featureList.map((f) => (
            <div key={f.key} className="flex items-center justify-between gap-4 py-2 border-b border-white/5 last:border-0">
              <div>
              ...
              <button
                ...
                className={`relative h-5 w-9 rounded-full transition-colors duration-200 shrink-0 ${
                  features[f.key] ? "bg-cyan-500" : "bg-white/10"
                }`}
              >
      ...
      <div className="rounded-[20px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,26,53,0.72)_0%,rgba(11,17,35,0.94)_100%)] p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-400">Status das Integrações</p>
        </div>
        <div className="space-y-3">
          {integrations.map((int) => (
            <div key={int.name} className="flex items-center justify-between gap-4 rounded-[12px] border border-white/6 bg-white/[0.025] px-4 py-3">
              <div>
